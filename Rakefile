...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/LinketySplit/linketysplit-ruby/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("linketysplit.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in linketysplit.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    replace_in_file "linketysplit.gemspec", /ruby_version = .*">= (.*)"/ => RubyVersions.lowest
